...
const PostCard = ({ post }) => {
  ...
  return (
    <div className="p-0 pb-12 mb-8 bg-white rounded-lg shadow-lg lg:p-8">
      <div className="relative mb-6 overflow-hidden shadow-md pb-72">
        <img
          src={post.featuredImage.url}
          alt={post.title}
          className="absolute object-cover w-full rounded-t-lg shadow-lg h-72 lg:rounded-lg"
        />
      </div>
      ...
      <p className="px-4 mb-3 text-lg font-normal text-center text-gray-700 lg:p-0">
        {post.excerpt}
      </p>

      <div className="text-center">
        <Link href={`/post/${post.slug}`}>
          <span className="inline-block px-8 py-3 text-lg font-medium text-white transition duration-500 transform bg-pink-600 rounded-full shadow-md cursor-pointer hover:-translate-y-1">
            Continue Reading
          </span>
        </Link>
      </div>
